...
import { ShoppingCart, Package, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
export default function PurchaseOrdersPage() {
  ...
  const [lineItem, setLineItem] = useState<PurchaseOrderLine>({
    productName: '',
    description: '',
    quantity: 1,
    unitPrice: 0,
  ...
  const handleConfirmOrder = async (id: number) => {
    try {
      await fetch(`http://localhost:8080/api/purchase-orders/${id}/confirm`, {
        method: 'POST'
      });
      fetchPurchaseOrders();
    } catch (error) {
  ...
  const handleMarkReceived = async (id: number) => {
    try {
      await fetch(`http://localhost:8080/api/purchase-orders/${id}/receive`, {
        method: 'POST'
      });
      fetchPurchaseOrders();
    } catch (error) {
  ...
  const handleCancelOrder = async (id: number) => {
    try {
      await fetch(`http://localhost:8080/api/purchase-orders/${id}/cancel`, {
        method: 'POST'
      });
      fetchPurchaseOrders();
    } catch (error) {
  ...
  const totalDraft = purchaseOrders.filter(o => o.status === 'Draft').length;
  const totalConfirmed = purchaseOrders.filter(o => o.status === 'Confirmed').length;
  const totalReceived = purchaseOrders.filter(o => o.status === 'Received').length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="card bg-base-200 shadow-md">
          <div className="card-body">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm opacity-70">Received</p>
          ...
          <tbody>
            {filteredOrders.map((order) => (
              <tr key={order.id}>
                ...
                <td>{order.expectedDeliveryDate || 'N/A'}</td>
